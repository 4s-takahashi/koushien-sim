 ...
 *   省略時は従来通りの挙動
 * @param externalTarget Phase 3: 外部から注入するターゲットコース（省略可）
 *   指定された場合、コース選択ロジックをスキップしてこの値を target として使用する
 */
export function selectPitch(
  ...
  // Phase 3: externalTarget が指定されている場合はコース選択ロジックをスキップ
